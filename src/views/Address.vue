<template>
    <div>
      ...
      <modal v-bind:mdShow="isMdShow" v-on:close="closeModal">
        <p slot="message">
          是否要确认删除此地址？
        </p>
        <div slot="btnGroup">
          <a href="javascript:void(0)" class="btn btn--m" @click="delAddress">确认</a>
          <a href="javascript:void(0)" class="btn btn--m" @click="isMdShow=false">关闭</a>
        </div>
      </modal>
      ...
      methods:{
        init(){
          axios.get("/users/addressList").then((response)=>{
            ...
            this.selectedAddrId = res.result[this.checkIndex].addressId
